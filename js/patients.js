...
(function (window, document) {
    ...
    function savePatient() {
        ...
        if (!sex) {
          ...
          /* Billing is parallel administration. Every patient reaches the
              nurse first, regardless of urgency or payment status. */
          var goesStraightToQueue = true;
          var status = STATUS.NURSE_TRIAGE;
    ...
    function performSave(payload, goesStraightToQueue) {
        if (performSave.inProgress) return;
        performSave.inProgress = true;
        if (ui.loading) ui.loading(true, 'Saving patient record…');
        ...
        var assessment = clinical.assess(saved.vitals);
          ...
          /* Every registration raises a queue-card bill on the billing desk,
              but the patient is never blocked from nurse triage by payment. */
        var regBill = null;
        if (!existing) {
        ...
        if (goesStraightToQueue) {
            window.MediTrackNotify.push(
                regBill ? 'Sent to nurse triage · billing remains open' : 'Sent to nurse triage',
                saved.name + ' is waiting for nurse triage before the doctor queue.' +
        ...
        /* Let the browser paint the progress state even though the local cache
           write is synchronous; the server persistence continues in the
           background through the store transport. */
        setTimeout(function () {
            performSave.inProgress = false;
            if (ui.loading) ui.loading(false);
        }, 220);
    }
